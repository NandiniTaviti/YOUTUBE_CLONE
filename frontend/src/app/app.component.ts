...
export class AppComponent implements OnInit, OnDestroy {
  ...
  loadSuggestions() {
    fetch("http://localhost:5000/api/youtube/search?q=top songs")
      .then(res => res.json())
  ...
  search() {
    ...
    fetch(`http://localhost:5000/api/youtube/search?q=${encodeURIComponent(this.query)}`)
      .then(res => res.json())
      .then(data => this.songs = data);
  }
  ...
  get progressBarStyle() {
    return { background: `linear-gradient(to right, #1db954 0%, #1db954 ${this.progress}%, #ccc ${this.progress}%, #ccc 100%)` };
  }
  ...
  getSongTitle(title: string) {
    return title.split('|')[0].replace(/full video song|video song|official|lyrical|hd/gi, '').trim();
  }

  toggleFavorite(song: any) {
    const index = this.favorites.findIndex(f => f.id.videoId === song.id.videoId);
    if (index === -1) this.favorites.push(song);
    else this.favorites.splice(index, 1);
    localStorage.setItem('favorites', JSON.stringify(this.favorites));
  }

  isFavorite(song: any) {
    return this.favorites.some(f => f.id.videoId === song.id.videoId);
  }
}